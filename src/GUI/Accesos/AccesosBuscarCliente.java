...
import Contenedores.AccesosCliente;
...
public class AccesosBuscarCliente extends javax.swing.JDialog {
    
    Eventos evento = new Eventos();
    private List<AccesosCliente> cliente;
    private int filas = 0;
    private String nombre,correo, plan, accesos, fecini, fecfin;
    private boolean flag = false;

    public AccesosBuscarCliente(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    ...
    public void SeleccionarCliente(){
        ...
        if(row < 0){
            ...
            plan = cliente.get(row).getPlan();
            accesos = cliente.get(row).getAccesos();
            fecini = cliente.get(row).getFecInicio();
            fecfin = cliente.get(row).getFecFinal();
            flag = true;
            this.dispose();
        }
    }
    ...
    public void LlenarTabla(){
        ...
        Call<List<AccesosCliente>> call = api.getClientesAcceso();
        call.enqueue(new Callback<List<AccesosCliente>>() {
            @Override
            public void onResponse(Call<List<AccesosCliente>> call, Response<List<AccesosCliente>> rspns) {
                cliente = rspns.body();
                ...
                while(filas < cliente.size()){
                    ...
                    tabla.setValueAt(cliente.get(filas).getApellidos()+" "+cliente.get(filas).getNombre(), filas, 1);
                    filas++;
                }
            }

            @Override
            public void onFailure(Call<List<AccesosCliente>> call, Throwable thrwbl) {
